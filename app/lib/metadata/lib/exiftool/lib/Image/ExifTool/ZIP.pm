        ...
        if ($mType) {
            ...
            if (not $status and $mime =~ /([\x21-\xfe]+)/s) {
                ...
                for (;;) {
                    ...
                    ($buff, $status) = $zip->contents($meta);
                    last if $status;
                    # quick hack to make EPUB .opf file look more like XMP
                    $buff =~ s{<package\b([^>]*)>}{<rdf:RDF><rdf:Description$1>}s;
                    $buff =~ s{</package>}{</rdf:Description></rdf:RDF>};
                    $buff =~ s{<(/?)metadata\b}{<${1}rdf:Description}g;
